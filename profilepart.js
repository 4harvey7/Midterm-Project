...
function longestWord() {
    var word1 = prompt("Enter the first word:").trim();
    var word2 = prompt("Enter the second word:").trim();
    if (!word1 || !word2) {
...
function knowBirthstone() {
    var month = prompt("Enter your birth month (Ex:January):").toLowerCase().trim();
    var birthstones = {
        january: "Garnet",
        february: "Amethyst",
        march: "Aquamarine",
    ...
    if (birthstones[month]) {
        ...
        alert("Invalid month. Please enter a valid month or Input the whole month name (ex: January) not (ex: Jan or 1).");
    }
}
